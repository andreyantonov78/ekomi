...
class subscribeModelScs extends modelScs {
	...
	private function _sendNewUserNotification($block, $userId, $password) {
		...
		if($emailSubject && $emailContent) {
			...
			frameScs::_()->getModule('mail')->send(get_bloginfo('admin_email'),
	...
	public function sendWpUserConfirm($username, $email, $confirmHash, $block) {
		$blogName = wp_specialchars_decode(get_bloginfo('name'));
		$replaceVariables = array(
			'sitename' => $blogName,
			'siteurl' => get_bloginfo('wpurl'),
