<?php
abstract class modelScs extends baseObjectScs {
	...
	public function getModule() {
		return frameScs::_()->getModule( $this->_code );
	}
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameScs::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
				return true;
			} else 
	...
	public function delete($params = array()) {
		if(frameScs::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else 
			$this->pushError (__('Database error detected', SCS_LANG_CODE));
		return false;
	...
	public function insert($data) {
		...
		$id = frameScs::_()->getTable( $this->_tbl )->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameScs::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	...
	public function update($data, $where) {
		...
		if(frameScs::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameScs::_()->getTable( $this->_tbl )->getErrors());
		return false;
	...
	public function exists($value, $field = '') {
		return frameScs::_()->getTable( $this->_tbl )->exists( $value, $field );
	}
	public function getFieldById($id, $field) {
		return frameScs::_()->getTable( $this->_tbl )->get($field, array($this->_idField => $id), '', 'one');
	}
}
