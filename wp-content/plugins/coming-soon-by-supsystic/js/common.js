...
/*function scsMceOnShowMoveMainToolbar(e, editor) {
	var panel = editor.theme.panel
	,	prevRect = panel.layoutRect();
	if(typeof(panel._scsOriginalTop) === 'undefined') {
		panel._scsOriginalTop = prevRect.y;
	}
	// If we showld move it- but it is not in original position	- move it back
	if(prevRect.y != panel._scsOriginalTop) {
		panel.scsSetCanMove();
		panel.moveTo(prevRect.x, panel._scsOriginalTop);
		panel._scsRefreshedOriginal = true;
		//panel._scsOriginalTop = prevRect.y;
	}
	var panelRect = panel.layoutRect()
...
function scsMceOnHideMoveMainToolbar(e, editor) {
	var panel = editor.theme.panel;
	if(!editor._scsHidingForMove && !panel._scsRefreshedOriginal) {
		...
		scsMceMoveToolbar(editor, newX);
	}
	editor._scsHidingForMove = false;
	panel._scsRefreshedOriginal = false;
	return;
	var panel = editor.theme.panel
...
function scsMceOnShowSubMenu(e, editor) {
	...
	,	controlHeight = jQuery('#'+ e.control._id).height()
	,	newX = rootRect.x
	,	newY = (panel._scsOriginalTop ? panel._scsOriginalTop : rootRect.y) - controlHeight + 10;
	
	if(panelOnBottom) {
		newY += controlHeight + 20;
		jQuery('#'+ e.control._id).addClass('mce-tinymce-subpanel-bottom');
	}
	e.control.scsSetCanMove();
	...
	editor._scsLastOpenedMenu = e.control;
}*/
