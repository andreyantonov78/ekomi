<?php
class supsystic_promoModelScs extends modelScs {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "'. dbScs::escape($code). '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbScs::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(SCS_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(SCS_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbScs::get($query, 'one');
	}
