...
scsElement_timer.prototype.initFinishDate = function() {
	this._progressTime = ((new Date( _scsGetCanvas().getParam('maint_end') )).getTime() - (new Date()).getTime()) / 1000;
};
...
scsElement_timer.prototype._updateTimer = function() {
	...
	for (var k in this._$helpers) {
		if (!this._$helpers.hasOwnProperty(k) || !this._$helpers[k].$root.is(':visible')) continue;
		...
		if (d[k] != null)
			this._$helpers[ k ].$num.html( this._formatTimeNum(parseInt(d[k])) );
	}
